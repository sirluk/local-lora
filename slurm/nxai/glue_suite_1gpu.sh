#!/bin/bash
#SBATCH --job-name=glue-suite-1gpu
...
# Hugging Face caches (adjust if you prefer a shared cache location)
...
# Example:
#   sbatch --export=ALL,WANDB_MODE_ARG=online slurm/glue_suite_1gpu.sh
...
python run_glue_suite.py \
  ...
  --output_root "runs/glue_suite_${SLURM_JOBID:-interactive}" \
