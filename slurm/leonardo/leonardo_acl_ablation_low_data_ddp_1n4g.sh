...
# Ablation A4: low-data regime on {rte,mrpc,cola} with max_train_samples sweep.
###############################################################################
#SBATCH --job-name=acl-lowdata-ddp-1n4g
...
TASKS="${TASKS:-rte,mrpc,cola}"
SEEDS="${SEEDS:-0,1,2}"

OUTPUT_ROOT="${OUTPUT_ROOT:-runs/leonardo_acl_low_data_${SLURM_JOBID:-interactive}}"

PER_DEVICE_TRAIN_BS="${PER_DEVICE_TRAIN_BS:-4}"
PER_DEVICE_EVAL_BS="${PER_DEVICE_EVAL_BS:-8}"
GRAD_ACCUM="${GRAD_ACCUM:-1}"

# Data regimes. The literal token "full" omits the flag (uses full training set).
...
for regime in ${DATA_REGIMES}; do
  OUT_REG="${OUTPUT_ROOT}/train_${regime}"
  mkdir -p "${OUT_REG}"

  EXTRA_ARGS=()
  if [ "${regime}" != "full" ]; then
    EXTRA_ARGS+=( --max_train_samples "${regime}" )
  fi

  torchrun --standalone --nproc_per_node="${NPROC}" --master_port="${MASTER_PORT}" run_glue_suite.py \
    --run_tag "${RUN_TAG}_${regime}" \
    ...
    --output_root "${OUT_REG}" \
    --results_csv "${OUT_REG}/results.csv" \
  ...
  python summarize_glue_results.py --results_csv "${OUT_REG}/results.csv" || true
done
