...
# Stage 1 (Protocol lock): 1B, seed=0, tasks {sst2,mrpc,rte,cola}
# Targets: attention-only (q_proj,v_proj).
###############################################################################
#SBATCH --job-name=acl-proto-attn-ddp-1n4g
...
OUTPUT_ROOT="${OUTPUT_ROOT:-runs/leonardo_acl_protocol_lock_attn_${SLURM_JOBID:-interactive}}"
METHODS="${METHODS:-all}"
...
torchrun --standalone --nproc_per_node="${NPROC}" --master_port="${MASTER_PORT}" run_glue_suite.py \
  ...
  --seeds 0 \
  --learning_rates "${LORA_LRS}" \
  --full_ft_learning_rates "${FT_LRS}" \
