...
# Ablation A3: head-aligned vs random grouping (and contiguous baseline).
# Runs the same suite multiple times with different --grouping_mode.
###############################################################################
#SBATCH --job-name=acl-grouping-ddp-1n4g
...
TASKS="${TASKS:-mrpc,rte,cola}"
SEEDS="${SEEDS:-0,1,2}"

OUTPUT_ROOT="${OUTPUT_ROOT:-runs/leonardo_acl_grouping_${SLURM_JOBID:-interactive}}"

PER_DEVICE_TRAIN_BS="${PER_DEVICE_TRAIN_BS:-4}"
PER_DEVICE_EVAL_BS="${PER_DEVICE_EVAL_BS:-8}"
GRAD_ACCUM="${GRAD_ACCUM:-1}"

# Compare these modes; head_aligned may error if divisibility constraints fail.
...
SCALING_MODE="${SCALING_MODE:-standard}"
...
for gm in ${GROUPING_MODES}; do
  ...
  torchrun --standalone --nproc_per_node="${NPROC}" --master_port="${MASTER_PORT}" run_glue_suite.py \
    ...
    --output_root "${OUT_GM}" \
    --results_csv "${OUT_GM}/results.csv" \
  ...
  python summarize_glue_results.py --results_csv "${OUT_GM}/results.csv" || true
done
