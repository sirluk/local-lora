...
submit_job() {
  ...
  local export_spec="ALL,OUTPUT_ROOT,DATA_REGIMES,MAX_EVAL_SAMPLES,LORA_LR,FT_LR,MAX_LENGTH,WARMUP_RATIO,SCALING_MODE,TARGET_SUFFIXES,M_VALUES,BD_N_VALUES,BD_ROW_FACTOR"
  local -a sbatch_args=(
    --parsable
    --time "${JOB_TIME}"
    --job-name "${job_name}"
