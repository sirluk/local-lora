...
# Ablation A1: BD-LoRA row-factor choice on {mrpc,rte,cola}:
#   bd_row_factor ∈ {block_a, block_b, dense}
###############################################################################
#SBATCH --job-name=acl-row-factor-ddp-1n4g
...
OUTPUT_ROOT="${OUTPUT_ROOT:-runs/leonardo_acl_row_factor_${SLURM_JOBID:-interactive}}"
...
for factor in ${ROW_FACTORS}; do
  ...
  torchrun --standalone --nproc_per_node="${NPROC}" --master_port="${MASTER_PORT}" run_glue_suite.py \
    ...
    --output_root "${OUT_FACTOR}" \
    --results_csv "${OUT_FACTOR}/results.csv" \
  ...
  python summarize_glue_results.py --results_csv "${OUT_FACTOR}/results.csv" || true
done
